...
def api_delete_single_alerta(alerta_id):
    try:
        conn = obtenerconexion()
        if not conn:
        ...
        with conn:
            with conn.cursor() as cursor:
                ...
                cursor.execute("UPDATE alertas_quiebre SET activo=0 WHERE id=%s", (alerta_id,))
                registrar_historial(conn, row['producto_id'], 'DELETE', motivo='Alerta descartada manualmente')
            conn.commit()
        return jsonify({'success': True, 'message': 'Alerta descartada correctamente'})
    except Exception as e:
    ...
    try:
        ...
        with conn:
            with conn.cursor() as cursor:
                ...
                cursor.execute("""
                    SELECT a.id AS alerta_id, a.producto_id, a.producto, a.sku, a.categoria, 
                           p.stock_total, p.venta_dia AS static_venta_dia, a.estado_transf
                ...
                for a in alertas_activas:
                    ...
                    # Combinamos la predicción con los datos de la alerta
                    prediccion_item = {
                        ...
                        'venta_dia_estatico': float(a['static_venta_dia'] or 0),
                        'venta_dia_real': pred['venta_dia_real'],
                        'horas_restantes_real': pred['horas_restantes_real'],
                        'nivel_real': pred['nivel_real'],
                        'usando_historial': pred['usando_historial'],
                        'estado_transf': a['estado_transf']
                    }
                    predicciones.append(prediccion_item)
                    
        return jsonify({'success': True, 'predicciones': predicciones})
    except Exception as e:
        return jsonify({'success': False, 'error': repr(e)}), 500

@app.route('/api/alertas/bulk-descartar', methods=['POST'])
def api_bulk_descartar_alertas():
    try:
        data = request.get_json() or {}
        alert_ids = data.get('alert_ids', [])
        
        if not alert_ids:
            return jsonify({'success': False, 'message': 'No se especificaron IDs de alertas'}), 400
        ...
        with conn:
            with conn.cursor() as cursor:
                # Obtener info de usuario de la sesión
                if 'codigo_empleado' in session:
                    cursor.execute("SELECT id FROM usuarios WHERE codigo_empleado=%s", (session['codigo_empleado'],))
                    u_row = cursor.fetchone()
                    if u_row:
                        usuario_id = u_row['id']
                
                # Desactivar cada alerta
                for aid in alert_ids:
                    cursor.execute("SELECT producto_id, producto FROM alertas_quiebre WHERE id=%s", (aid,))
                    row = cursor.fetchone()
                    if row:
                        prod_id = row['producto_id']
                        # Desactivar alerta
                        cursor.execute("UPDATE alertas_quiebre SET activo=0 WHERE id=%s", (aid,))
                        ...
                        cursor.execute("""
                            INSERT INTO historial_ajustes
                                (producto_id, usuario_id, empleado_nombre, accion,
                                 campo_modificado, valor_anterior, valor_nuevo, motivo)
                            VALUES (%s, %s, %s, 'DELETE', NULL, NULL, NULL, %s)
                        """, (prod_id, usuario_id, usuario_nombre, f'Alerta descartada en lote'))
                        
                conn.commit()
                
        return jsonify({'success': True, 'message': f'{len(alert_ids)} alertas descartadas correctamente'})
    except Exception as e:
        return jsonify({'success': False, 'error': repr(e)}), 500
    ...
    try:
        ...
        with conn:
            with conn.cursor() as cursor:
                ...
                """)
                stock_cat = cursor.fetchall()
                
                # 2. Tendencia de ajustes (últimos 7 días)
                cursor.execute("""
                ...
                cursor.execute("""
                    ...
                    GROUP BY nivel
                """)
                alertas_nivel = cursor.fetchall()
                
        return jsonify({
            'success': True,
            'stock_por_categoria': stock_cat,
            'tendencia_ajustes': tendencias,
            'alertas_por_nivel': alertas_nivel
        })
    except Exception as e:
        return jsonify({'success': False, 'error': repr(e)}), 500
